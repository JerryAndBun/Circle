    ...
    <div class="selected_area" v-if="selected">
      <div class="video_info">
        ...
        <div class="cover">
          <img
            v-if="have_preview"
            class="video_cover_preview"
            :src="`${baseurl}${video_cover_url}`"
            alt=""
            @click="chosecover"
          />
          <div class="no_cover" @click="chosecover">
            ...
            <input
              ref="upload_cover_input"
              class="upload_cover_input"
              type="file"
              accept="image/*"
              @change="uploadImg"
            />
...
export default {
  ...
  methods: {
    ...
    uploadImg(e) {
      ...
      /**
       * FileReader是一个文件读取器，他有自己的事件，比如说load，loadend这些事件
       * 下面就相当于监听这个reader的读取事件，当被读取的时候执行我们写好的函数
       * 传进来是个file文件，然后形参是e，就是DOM的那个e，所以拿到文件需要e.target.result
       * 然后e.target.result又是一个arrayBuffer格式的文件，我们要展示缩略图必须要file或者Blob格式
       * 所以需要穿进Blob函数转一下格式再传给window.URL.createObjectURL，用于创建缩略图的链接
       * @param {*} e
       */
      reader.onload = (e) => {
        let data
