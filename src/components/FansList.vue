<template>
  <div>
    <div class="togle">
      <div class="page1" @click="topage1">粉丝列表</div>
      <div class="page2" @click="topage2">关注列表</div>
      <div class="line" ref="line"></div>
    </div>
    <!-- <router-view :itemList='itemList'></router-view> -->
    <div class="backgroundDiv">
      <div v-if="isempty" class="emptyinfo">
        <img src="../assets/imgs/这里什么都没有.png" class="empty" alt="" />
        <div class="emptytext" id="emptytext" ref="emptytext">暂无粉丝，快去投稿吧~</div>
      </div>
      <div class="fansList" v-if="ispage1">
        <UserPreview v-for="(item, index) in fansList" :key="index" :item="item"></UserPreview>
      </div>
      <div class="focusList" v-if="ispage2">
        <UserPreview v-for="(item, index) in focusList" :key="index" :item="item"></UserPreview>
      </div>
    </div>
  </div>
</template>

<script>
import { BASE_URL } from '../api/config'
import HttpManager from '../api/index'
import { mapGetters } from 'vuex'
import UserPreview from '../components/UserPreview.vue'

export default {
  props: ['itemList'],
  data() {
    return {
      ispage1: 1,
      ispage2: 0,
      isempty: 0,
      item_list: '',
      fansList: '',
      focusList: '',
    }
  },
  components: {
    UserPreview,
  },
  methods: {
    topage1() {
      HttpManager.getFansListByUid(`/fansList/${this.$route.params.myuid}`).then(
        (response) => {
          this.fansList = response
          if (this.fansList.length == 0) {
            this.isempty = 1
            this.$nextTick(() => {
              console.log((this.$refs.emptytext.innerHTML = '暂无粉丝，快去投稿吧~'))
            })
          } else {
            this.isempty = 0
          }
          if (!this.ispage1) {
            this.$refs.line.style.transform = 'translateX(-64px)'
          }
          this.ispage1 = 1
          this.ispage2 = 0
        },
        (error) => {
          console.log('获取粉丝列表失败')
        }
      )
    },
    topage2() {
      HttpManager.getFansListByUid(`/focusOnList/${this.$route.params.myuid}`).then(
        (response) => {
          this.focusList = response
          if (this.focusList.length == 0) {
            this.isempty = 1
            this.$nextTick(() => {
              console.log((this.$refs.emptytext.innerHTML = '暂无关注的人哦'))
            })
          } else {
            this.isempty = 0
          }
          if (this.ispage1) {
            this.$refs.line.style.transform = 'translateX(64px)'
          }
          this.ispage1 = 0
        },
        (error) => {
          console.log('获取关注列表失败')
        }
      )
      this.ispage2 = 1
    },
  },
  created() {
    this.item_list = this.itemList
    this.topage1()
  },
}
</script>

<style lang="scss" scoped>
@import '../assets/css/fanslist.scss';
</style>
