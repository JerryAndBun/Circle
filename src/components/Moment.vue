<template>
  <div>
    ...
    >
      ...
      <div
        class="reasonClickformore"
        ref="reasonClickformore"
        @click="unfold('reason')"
        v-if="reasonClickformore"
      >
        展开
      </div>
      <div class="bottom">
        ...
        <div class="forward" @click="openForwardWindow">
          ...
          >{{ item.forwardVideoInfo == null ? 0 : item.forwardVideoInfo.length }}
        </div>
      </div>
      <div class="commentArea" v-if="isCommentDiv">
        ...
        <CommentInput
          :comment_level="1"
          class="commentInput"
          @sendComment="momentCommentRequest"
        ></CommentInput>
        <CommentDiv
          class="commentDiv"
          ref="commentDiv"
          v-for="(comment, index) in momentComment"
          :key="index"
          :level_item="comment"
          :momentItem="item"
          :is_all="isAll"
    ...
    <div class="forwardMoment" v-if="item.type == 'SHARE_DYNAMIC_CONTENT'">
      ...
      <main class="forwardContent">
        <div class="userDiv">
          ...
          <div
            class="nickName"
            v-for="(element, index) in item.forwardVideoInfo.forwardUserInfoList"
          >
            &nbsp昵称{{ element.nickname }}
          </div>
        </div>
        <div class="textContainer" ref="textContainer" id="textContainer">
          <div class="forwardInner" ref="forwardInner">
            <span class="forwardText" ref="forwardText"> </span>
          </div>
        </div>
        <div
          class="forwardClickformore"
          ref="forwardClickformore"
          @click="unfold('forward')"
          v-if="forwardClickformore"
        >
          展开
        </div>
      </main>
        ...
        <div class="videoWarp">
          ...
          <aside class="videoInfo">
            ...
            <div class="icons">
              ...
              {{ 321 }}播放 &nbsp &nbsp <i class="iconfont icon-pinglun"></i>&nbsp{{ 321 }}评论
            </div>
          </aside>
        </div>
      </main>
    ...
    >
      ...
      <main class="conVideoContent" @click="toThisVideo">
        ...
        <aside class="videoInfo">
          ...
          <div class="icons">
            <i class="iconfont icon-bofangliang" @click="isCommentDiv = !isCommentDiv"></i>&nbsp{{
              item.videoNoteDto.playNum
            }}播放 &nbsp &nbsp <i class="iconfont icon-pinglun"></i>&nbsp{{
              item.videoNoteDto.commentNum
            }}评论
          </div>
        </aside>
      </main>
